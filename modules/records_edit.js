export const data = {
  title: "{{ this.data.domain.nopunycode }}",
  domain: 0,
  record: 0
};

export const funcs = {
  onMount: async function() {

    if(await this.main.funcs.checkSession()) {

      if(this.params.name && this.params.rid) {

	const d = this.main.funcs.sendIni(`domains/${this.params.name}`);
	fetch(d.url, d.options).then(r => {
	  if(this.main.funcs.sendEnd(r))
	    return r.json();
	}).then(j => {
	  if(j.status === 200) {
	    if(j.domain.readonly)
	      return this.link('/domains');

	    this.data.domain = j.domain;
	    if(!(this.data.record = j.records.find(({ id }) => id == this.params.rid)))
	      return this.link('/domains');

	    this.data.record.name = this.data.record.name.slice(0, this.data.record.name.length - this.data.domain.name.length).replace(/\.$/, '');
	    this.e('frecords').innerHTML = this.main.funcs.records(this.data.record.content);
	  } else
	    this.toastShow(j.message || this.text(`err${j.status}`));
	}).catch(err => {
	  this.idle(false);
	  this.toastShow(err);
	});

      } else
	this.link('/domains');

    } else
      this.main.funcs.logout();
  },

  modify: async function() {
    let name = this.e('name').value.trim();
    name+= ((name === '') ? '' : '.') + this.data.domain.name;
    let data = {name: name, type: this.data.record.type, content: this.main.funcs.recordfrm(), ttl: this.data.record.ttl, disabled: this.data.record.disabled};
    if(this.e('no-ip').checked && this.e('newno-ip').checked) data.no_ip = true;
    if(!this.e('no-ip').checked) data.no_ip = false;
    if(await this.main.funcs.send(`domains/${this.data.domain.name}/records/${this.params.rid}`, 'POST', data)) {

      this.toastShow(this.text('updated'));
      this.main.funcs.wsocket_send({m: 'records', a: 'edit', id: this.data.domain.id});
      this.link(`/domains/${this.data.domain.name}/records`);
    }
  }
};

export const html = `<h1><i class="fa-solid fa-network-wired"></i>
<span>{{ this.data.domain.name }}</span> - <span>{{ this.data.record.name }}</span>
<div style="float: right"><a :href="/domains/{{ this.data.domain.name }}/records" is="jpau-link"><button class="button" style="min-width: max-content; font-size: .52em"><i class="fa-solid fa-arrow-left"></i><br><b>{{ this.text('back') }}</b></button></a></div>
</h1>

<div style="width: 420px" class="box">
  <form method="post" @submit="this.funcs.modify()">
    <div class="tc">
      <div>{{ this.text('name') }}</div>
      <input type="text" id="name" size="10" maxlength="200" *value="this.data.record.name" class="txt"> .<span>{{ this.data.domain.name }}</span>
    </div><div class="tc">
      <div class="ib vt" style="padding: 0 6px">
	<div>{{ this.text('type') }}</div>
	<select id="type">
	  <option data-jpau-for="for(let type in this.main.data.types)" :value="{{ type }}" *selected="this.data.record.type">{{ type }}</option>
	</select>
      </div><div class="ib vt" style="padding: 0 6px">
	<div>{{ this.text('ttl') }}</div>
	<input type="number" id="ttl" size="8" min="30" max="4294967295" step="1" *value="this.data.record.ttl" class="txt">
      </div>
    </div>
    <div id="frecords"></div>
    <div class="tc"><br>
      <div class="ib vt" style="padding: 0 6px"><input type="checkbox" id="disabled" *checked="this.data.record.disabled" class="switch"><label for="disabled">{{ this.text('disabled') }}</label></div>
      <div class="ib vt" style="padding: 0 6px"><input type="checkbox" id="no-ip" *checked="this.data.record.no_ip" class="switch"><label for="no-ip">{{ this.text('no_ip') }}</label><br><input type="checkbox" id="newno-ip" *disabled="!this.data.record.no_ip" class="switch"><label for="newno-ip" class="sm">{{ this.text('new') }}</label></div>
    </div><br>

` + june_pau.main.funcs.buttons() + `
  </form>
</div>
`;
